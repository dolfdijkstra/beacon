(function(w) {
	...
	var beacon = {
		...
		send_json : function(data) {
			...
			sendRequest("beacon", JSON.stringify(data));
		},
		...
		collectPerformanceData : function() {
			...
			if (w.performance && w.performance.timing)
				this.copyProperties(w.performance.timing, p.timing);
			if (w.performance && w.performance.navigation)
				this.copyProperties(w.performance.navigation, p.navigation);
			this.copyProperties(w.screen, p.screen);
			...
			p.domCount = document.getElementsByTagName("*").length;
			p.imgCount = document.getElementsByTagName("img").length;
			p.linkCount = document.getElementsByTagName("link").length;
			p.scriptCount = document.getElementsByTagName("script").length;
	...
	setTimeout(function() {
		beacon.send_json()
	}, 5); // wrap send_json call in function, because FF sends lateness
